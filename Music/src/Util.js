...
class Util {
    ...
    static getVideoBySearch(search, options = {}, queue, requestedBy) {
        return new Promise(async (resolve, reject) => {
            ...
            if (isVideoLink) {
                ...
                let VideoID = (await require('ytdl-core').getInfo(search)).videoDetails.videoId;
     ...
     */
     static buildBar(value, maxValue, size, loadedIcon, behindIcon, arrowIcon) {
        const percentage = value / maxValue;
        const progress = Math.round((size * percentage));
        const emptyProgress = size - progress;

        const progressText = behindIcon.repeat(progress) + arrowIcon;
